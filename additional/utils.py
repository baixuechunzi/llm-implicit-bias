# from https://raw.githubusercontent.com/chadaeun/weat_replication/master/lib/weat.py
...
def get_embedding(path, layer):
    loaded = np.load(path)
    # Access arrays by index (e.g., 'arr_0', 'arr_1', etc.)
    return loaded[f'arr_{layer}']
...
def csv_to_dict(file_path):
    ...
    with open(file_path, mode='r', encoding='utf-8') as file:
        ...
        # Get the header (first line) to use as keys
        header = next(csv_reader)

        # Initialize the dictionary with empty lists for each key
        result = {key: [] for key in header}

        # Iterate over the remaining rows and append values to corresponding keys
